...
class NeuralNetwork:
    def __init__(self,args):
        ...
        prev = 784
        for sz in h:
            layers.append(NNLayer(sz,prev,init=wi))
            layers.append(ActivationLayer(self._activation))
            prev = sz
        layers.append(NNLayer(10,prev,init=wi))
        self.layers = layers
    ...
    def backward(self,y,y_hat):
        if y.ndim==1 or (y.ndim==2 and min(y.shape)==1):
            ...
            yoh[np.arange(len(yf)),yf] = 1.0
            y = yoh
        self.loss_fn.forward_pass(y_hat.T,y.T)
        g = self.loss_fn.backward_pass()
        gW = []
        gb = []
    ...
    def train(self,X_train,y_train,epochs=1,batch_size=32):
        ...
        for _ in range(epochs):
            ...
            for s in steps:
                ...
                total += self.loss_fn.forward_pass(yhat.T,yb)
